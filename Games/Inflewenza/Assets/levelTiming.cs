...
public bool fadeOut = false;	
	...
	void Update () 
	{
		...
		if(go)
		{
			...
			UILabel subtitle = GameObject.Find("Label1").GetComponent<UILabel>();
			
			subtitle.text = string.Format("Level Time: {0}\'{1}\"{2}", minutes, seconds, fraction);
	...
	void FadeIn()
	{
		...
		//UILabel.material.color = TimerColor;
		if(TimerColor.a == 1)
		{
			fadeIn = false;
		}
...
public bool fadeOut = false;	
	...
	void Update () 
	{
		...
		if(go)
		{
			...
			seconds = (int)(playTime % 60f);
			fraction =  (int)((playTime *10) %10);
			guiText.text = string.Format("Level Time: {0}\'{1}\"{2}", minutes, seconds, fraction);
		}
